...
class PlayerScoreboardMoveEditorView: UIView {
    ...
    fileprivate func fillUI() {
        ...
        viewModel.assistMoveCount.bindAndFire{[unowned self] in
            self.assistCountLabel.text = $0}
        viewModel.reboundMoveCount.bindAndFire{ [unowned self] in self.reboundCountLabel.text = $0}
        viewModel.foulMoveCount.bindAndFire{[unowned self] in
            self.foulCountLabel.text = $0}
    }
